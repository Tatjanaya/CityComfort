...
def read_class_map_info(img_path):
    ...
    del class_info
    return class_map_x,class_map_y,dGeoTrans2   
...
def fun_findpoint(i,j,dGeoTrans,dataset,dGeoTrans2):   #在分类图上找到L8影像上第（i，j）点
    ...
    result=fun_lat_to_line(temp[1],temp[0],dGeoTrans2)
    return result

def fun_cal_epsilon_veg(i,j,m_class,ndvi):    #计算植被土壤混合的发射率（NDVI加权）
...
def fun_epsilon(B10_path,B11_path,B4_path,B5_path,class_map_path):
    ...
    class_map_x,class_map_y,dGeoTrans2=read_class_map_info(class_map_path) #分类图数据，长，宽

    NIR=fun_normalizeDiffer(NIR,RED)   #NDVI

    epsilon1=np.zeros((height,width)) #10波段   #初始化发射率矩阵
    epsilon2=np.zeros((height,width)) #11波段

	#获取投影信息
    dGeoTrans  = dataset.GetGeoTransform()    #读入L8的信息
    start_point = fun_findpoint(startx,starty,dGeoTrans,dataset,dGeoTrans2) #找到分类图中起点位置
    class_info = gdal.Open(class_map_path)
    band=class_info.GetRasterBand(1)
    x_0=int(start_point[0])
    y_0=int(start_point[1])
    if(x_0+height<=class_map_y and y_0+width<=class_map_x):
        class_map=band.ReadAsArray(y_0,x_0,width,height)
    elif(x_0+height<=class_map_y):
        class_map=band.ReadAsArray(y_0,x_0,class_map_x-y_0,height)
    elif(y_0+width<=class_map_x):
        class_map=band.ReadAsArray(y_0,x_0,width,class_map_y-x_0)
    else:
        class_map=band.ReadAsArray(y_0,x_0,class_map_x-y_0,class_map_y-x_0)
    m_proj=dataset.GetProjection()
    del dataset
    del class_info
        #遍历获取发射率
    itertools.product(range(0,height),range(0,width))
    print("calculate epsilon")
    for item in itertools.product(range(0,height),range(0,width)):
        i=item[0]
        j=item[1]
        if (RED[item]==off_4):    #表示图像边缘的背景区域 不进行计算
            epsilon1[item[0]][item[1]]= np.nan 
            epsilon2[item[0]][item[1]]= np.nan 				
            continue
        x= int(round(item[0]+x_0))
        y= int(round(item[1]+y_0))    
        if(x>=class_map_y or y>=class_map_x or x<0 or y<0 ):               #不在分类图图范围内
            epsilon1[item[0]][item[1]]=np.nan 	
            epsilon2[item[0]][item[1]]=np.nan 	
            continue
        if(class_map[i,j]>4):
            epsilon1[item[0]][item[1]]=com_emiss[class_map[i,j]-1][0]   #com_emiss是给定的物体发射率数组
            epsilon2[item[0]][item[1]]=com_emiss[class_map[i,j]-1][1]			
        else:
            ep=fun_cal_epsilon_veg(item[0],item[1],class_map[i,j],NIR[i,j])
